...
from PersonCar.label_name import Chinese_name
...
class PersonCar_MainWindow(QBeautyUI):
    def __init__(self, *args, obj=None, **kwargs):
        super(PersonCar_MainWindow, self).__init__(*args, **kwargs)
        ...
        self.c_video = 0
        self.count_name = ["行人", '自行车', '小汽车', '摩托车', '公交车', '卡车']
        self.count_table = []
            ...
            if col == 0:  # 点击第一列时
                ...
                if (this_path is not None) & (res is not None) & (axes is not None):
                    ...
                    if os.path.exists(this_path):
                        ...
                        self.label_person.setText(str(count[0]))
                        self.label_car.setText(str(count[2]))
                        self.label_truck.setText(str(count[5]))
                        self.label_bus.setText(str(count[4]))
                        self.label_bicycle.setText(str(count[1]))
                        self.label_motorcycle.setText(str(count[3]))
        ...
        if fileName_choose != '':
            ...
            if det is not None and len(det):  # 如果有检测信息则进入
                ...
                for *xyxy, conf, cls in reversed(det):  # 遍历检测信息
                    ...
                    self.label_class_result.setText(str(self.names[int(cls)]))  # 显示类别
                    for cn in range(len(self.count_name)):
                        if self.names[int(cls)] == self.count_name[cn]:
                            count[cn] += 1
                    self.label_score_result.setText('%.2f' % conf)
